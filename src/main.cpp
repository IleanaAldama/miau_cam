...
int run() {
  ...
  if (!landmarkers) {
    std::cerr << "Failed to create landmarkers: " << landmarkers.error()
              << std::endl;
    return 1;
  }
  ...
  while (cap.read(frame) && !frame.empty()) {
    ...
    flow_log << std::fixed << std::setprecision(4) << now << ","
             << flow.magnitude << "," << flow.coherence << ","
             << state.spin.last_score_debug << ","
             << state.spin.last_fraction_debug << ","
             << state.spin.last_peak_debug << "," << to_string(current_gesture)
             << "\n";
  ...
  cap.release();
  for (auto &[gesture, vcap] : video_caps.value().caps)
    vcap.release();
