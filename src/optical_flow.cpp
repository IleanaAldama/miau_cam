...
FlowSignal compute_frame_flow(const cv::Mat& frame, cv::Mat& prev_small_gray) {
    ...
    cv::resize(gray, small, cv::Size(tuning::spin_flow_width, tuning::spin_flow_height));
    ...
    if (static_cast<double>(moving_count) / total < tuning::spin_flow_min_moving_fraction) {
        sig.coherence = 0.0;
        prev_small_gray = small;
        return sig;
    }
...
namespace {
void prune_older_than(std::vector<std::pair<double, double>>& v, double now_ms, double window_ms) {
...
SpinTrackerState update_spin_tracker(SpinTrackerState state, double magnitude, double coherence,
                                       double now_ms) {
    ...
    prune_older_than(state.flow_peak_history, now_ms, tuning::spin_flow_peak_hold_ms);
    ...
    for (const auto& [t, m] : state.flow_history) {
        if (m > tuning::spin_mag_threshold) elevated++;
    }
    state.last_fraction_debug =
        state.flow_history.empty() ? 0.0
